...
class AddBookCopiesToInventoryTest extends BaseInventoryTestController {
    ...
    void setUp() {
        super.setUp(
                ...
                this.bookstoreExceptionHandler,
    ...
    @DisplayName(
            "PUT "
                    ...
                    + " returns HTTP 200 with InventoryItemDto when copies are added successfully")
    ...
    void shouldReturn200AndInventoryItemDto_whenBookCopiesAddedSuccessfully(final Long copies)
        ...
        final InventoryItemDto expectedInventoryItemDto =
                this.givenInventoryItemDto(bookId, copies);
    ...
    @DisplayName(
            "PUT "
                    + ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH
                    + " returns HTTP 204 when no book copies are added")
    ...
    @DisplayName(
            "PUT "
                    + ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH
                    + " returns HTTP 500 when an unexpected error occurs")
    ...
    @DisplayName(
            "PUT "
                    + ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH
                    + " returns HTTP 4xx on client cases")
    class ClientCases {
        ...
        @DisplayName(
                "PUT "
                        + ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH
                        + " returns HTTP 400 when request body is invalid")
                    ...
                    AddBookCopiesToInventoryTest.this
                            ...
                            .perform(
                                    put(ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH, bookId)
                                            .contentType(MediaType.APPLICATION_JSON)
                                            .content(
                                                    AddBookCopiesToInventoryTest.this.objectMapper
                                                            .writeValueAsString(allocationDto)))
                            .andReturn();
        ...
        @DisplayName(
                "PUT "
                        + ADD_BOOK_COPIES_TO_INVENTORY_ENDPOINT_PATH
                        + " returns HTTP 422 when request body is empty")
